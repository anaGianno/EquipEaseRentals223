...
--EquipmentType,Rental,RentEquipment,Rental,RentEquipment,Customer
Select TypeName,startTime,returnTime,hireFrom,returnTo,Email
...
where Rental.CustomerEmail = 'bc245@students.waikato.ac.nz'
order by startTime

--Upcoming
Select EquipmentType.TypeName,startTime,returnTime,hireFrom,returnTo,Email
from rentEquipment join Rental
on rentEquipment.rRentalID = Rental.RentalID
join Customer on Customer.Email = Rental.CustomerEmail
join Equipment on Equipment.EquipmentID = rentEquipment.rEquipmentID
...
join Equipment on Equipment.EquipmentID = rentEquipment.rEquipmentID
join EquipmentType on EquipmentType.TypeName = Equipment.TypeName
where CURRENT_TIMESTAMP > startTime and returnTime is not null
order by startTime
...
join Equipment on Equipment.EquipmentID = rentEquipment.rEquipmentID
join EquipmentType on EquipmentType.TypeName = Equipment.TypeName
where CURRENT_TIMESTAMP > startTime and returnTime is null

--combo box 1: equipType
Select distinct TypeName 
from EquipmentType

--combo box 2: hireFrom
Select distinct hireFrom 
from Rental
...
--customer,rental,rentEquipment
--TypeName,startTime,hireFrom
CREATE TABLE Branch(
...
--1
Select count(distinct CustomerEmail) as 'Unique Customers', hireFrom from Rental where startTime like '%2023%' group by hireFrom

--2
Select distinct TypeName, sum(DateDiff(hour,startTime,returnTime))/count(DateDiff(hour,startTime,returnTime)) as 'Average rental duration' from
...
Select count(TypeName)as typeNameCount, TypeName,BranchName from Equipment
...
declare @auckland varchar(30) = 
	(Select top 1 TypeName from Equipment
	join rentEquipment on Equipment.EquipmentID=rentEquipment.rEquipmentID
	where BranchName = 'Auckland Central Branch'
	Group by TypeName,BranchName
	order by BranchName, count(TypeName) desc)

declare @christchurch varchar(30) = 
	(Select top 1 TypeName from Equipment
	join rentEquipment on Equipment.EquipmentID=rentEquipment.rEquipmentID
...
--4
Select Count(RentalID) as 'Total number of rentals' ,year_ = DATEPART(YEAR,startTime), month_ = DATEPART(MONTH,startTime) from Rental
...
select startTime from Rental
order by startTime 
--5
Select rEquipmentID,Equipment.TypeName,ratePerHour,DateDiff(hour,startTime,returnTime) as 'hours', ratePerHour*DateDiff(hour,startTime,returnTime) as 'total income' from EquipmentType
...
Select rEquipmentID,Equipment.TypeName,sum(ratePerHour)as'sum rate per hour' from EquipmentType
join Equipment on EquipmentType.TypeName = Equipment.TypeName
join rentEquipment on rentEquipment.rEquipmentID = Equipment.EquipmentID
join Rental on Rental.RentalID = rentEquipment.rRentalID
where startTime like '%2023%'
...
Select rEquipmentID,Equipment.TypeName,sum(DateDiff(hour,startTime,returnTime)) as 'hours' from EquipmentType
join Equipment on EquipmentType.TypeName = Equipment.TypeName
join rentEquipment on rentEquipment.rEquipmentID = Equipment.EquipmentID
join Rental on Rental.RentalID = rentEquipment.rRentalID
where startTime like '%2023%'
group by rEquipmentID,Equipment.TypeName,startTime,returnTime


Select rEquipmentID,Equipment.TypeName,sum(ratePerHour)as sumRatePerHour,sum(DateDiff(hour,startTime,returnTime)) as hours_,sum(ratePerHour)*sum(DateDiff(hour,startTime,returnTime)) as 'total'  from EquipmentType
join Equipment on EquipmentType.TypeName = Equipment.TypeName
join rentEquipment on rentEquipment.rEquipmentID = Equipment.EquipmentID
join Rental on Rental.RentalID = rentEquipment.rRentalID
where startTime like '%2023%'
...
Select rEquipmentID,EquipmentID, BranchName, rRentalID,RentalID, startTime, returnTime, hireFrom, returnTo
from Equipment join rentEquipment 
on Equipment.EquipmentID = rentEquipment.rEquipmentID
join Rental on Rental.RentalID = rentEquipment.rRentalID
where RentalID = '102'

Select EquipmentID,BranchName,RentalID,startTime
from Rental join Equipment
on Equipment.BranchName = Rental.hireFrom
where RentalID = '102'

--If that branch stocks that equipment
--If that equipment is available
Select TypeName, BranchName,available from Equipment
Select * from Equipment
where BranchName like 'Tauranga Downtown Branch'
Select * from Rental
Select * from Customer
Select * from rentEquipment
order by returnTime desc
Delete from Customer where fName = '1'
Select distinct email from Customer

Select distinct TypeName from Equipment
where TypeName not in (Select distinct TypeName from Equipment where BranchName = 'Auckland Central Branch')

Select SELECT * FROM Rental WHERE TypeName LIKE 

Select * from Rental
order by CustomerEmail

Select * from Rental
where CustomerEmail = 'bc245@students.waikato.ac.nz'
...
Update Equipment set available = '1' where BranchName = 'Auckland Central Branch' and typeName = 'Concrete Mixer'
Select max(RentalID) from Rental

Select * from rentEquipment where rRentalID = '105'
delete from rentEquipment where rRentalID = '102'
Select * from Equipment

Select typeName,max(purchaseDate) as 'Most Recent'
from Equipment
where BranchName = 'Auckland Central Branch'
group by typeName
order by max(purchaseDate) desc

Select typeName,purchaseDate from Equipment
order by purchaseDate desc

Select distinct EquipmentID from Equipment
Select  distinct RentalID from Rental
Select distinct returnTo from rentEquipment where returnTo != 'null'

insert into rentEquipment values ('1','102','2027-04-11 09:00:00','Auckland Central Branch')
Delete from rentEquipment  where rRentalID = '108'

Select * from rentEquipment

--
Select DATEDIFF(minute,startTime,returnTime) as 'Difference' from Rental join rentEquipment on
Rental.RentalID = rentEquipment.rRentalID 
where DATEDIFF(minute,startTime,returnTime)  <0

drop trigger updateAvailable
Create trigger updateAvailable
On rentEquipment
after Update as
begin
if exists (Select DATEDIFF(minute,startTime,returnTime) as 'Difference' from Rental join inserted on
Rental.RentalID = inserted.rRentalID 
where DATEDIFF(minute,startTime,returnTime)  <0) begin
	RAISERROR('Return time can not be earlier than start time',16,1)
	rollback tran
end
declare @branch varchar(30) = (Select returnTo from inserted)
if @branch is not null begin
Update Equipment set BranchName = @branch where  EquipmentID in (Select rEquipmentID from inserted)
Update Equipment set available = '1' where EquipmentID in (Select rEquipmentID from inserted)
end
end

--
drop trigger updateOff
Create trigger updateOff
On Rental
after insert as
begin
if exists (Select * from inserted where startTime < CURRENT_TIMESTAMP) begin
	RAISERROR('Cannot create a booking to start in the past',16,1)
	rollback tran
end
else begin 
	declare @branch varchar(30) = (Select hireFrom from inserted)
	Update Equipment set available = '0' where EquipmentID in 
	(Select rEquipmentID from inserted join rentEquipment on inserted.RentalID = rentEquipment.rRentalID)
	and BranchName in (Select hireFrom from inserted)

end
end

drop trigger setRentEquipment
create trigger setRentEquipment
On rentEquipment
after insert as 
begin
if exists (Select * from inserted where returnTime < CURRENT_TIMESTAMP) begin
	RAISERROR('Cannot begin to rent equipment in the past',16,1)
	rollback tran
end
Update rentEquipment
Set returnTime = null where returnTime in (Select returnTime from inserted)
Update Equipment set available = '0' where EquipmentID in 
(Select rEquipmentID from rental join inserted on Rental.RentalID = inserted.rRentalID)
and BranchName in (Select hireFrom from rental join inserted on Rental.RentalID = inserted.rRentalID)
end

drop trigger customerDupe
create trigger customerDupe
On customer 
after insert as 
begin
declare @email varchar(30) = (select email from inserted)
declare @x int = null
Select @x = count(email) from customer where email = @email
if @x >1 begin
...
Update Equipment set available = '1' where EquipmentID = '1'
Update Equipment set available = '1' where EquipmentID = '2'
begin tran
insert into rentEquipment values ('1','105','2027-04-11 09:00:00','Auckland Central Branch');
rollback tran

Select * from Equipment
Select * from rentEquipment
order by rRentalID desc
Select * from rentEquipment
order by rEquipmentID desc
Select * from Rental


Select EquipmentID from Equipment where BranchName like 'Auckland Central Branch'

Select RentalID from Rental where hireFrom like 

Select * from Rental where CustomerEmail = 'bc245@students.waikato.ac.nz'
Select * from Customer where email like 'bc245%'
Delete from Rental where CustomerEmail = 'bc245@students.waikato.ac.nz'
Select * from rentEquipment where rRentalID = '135'
Delete from rentEquipment where rRentalID = '1139' 
Delete from rentEquipment where rRentalID = '76'

Select * from rentEquipment where returnTo is null
order by rRentalID desc

Select rRentalID from rentEquipment where rEquipmentID = '1' and returnTime is null
order by rRentalID desc

Update rentEquipment set returnTime = '', returnTo= '' where rEquipmentID = '' and rRentalID = ''
Select * from rentEquipment where rEquipmentID = '124' and returnTime is null and returnTo is null

Select distinct rEquipmentID from rentEquipment where returnTime is null and returnTo is null
order by rEquipmentID
Select distinct rRentalID from rentEquipment where rEquipmentID = '124' and returnTime is null and returnTo is null
Select distinct branchName from Equipment where typeName in (Select typeName from Equipment where EquipmentID = '1')
--6
Select * from equipment where typeName = 'Concrete Mixer'
Select * from Rental where hireFrom in
Insert into 
Select * from rentEquipment where rEquipmentID = '62'

Select * from Equipment where BranchName like 'Auckland Central Branch'

Select distinct BranchName from Equipment left outer join rentEquipment on Equipment.EquipmentID = rentEquipment.rEquipmentID
left outer join Rental on Rental.RentalID = rentEquipment.rRentalID
where typeName = 'Concrete Mixer'

--
Select distinct hireFrom from Equipment left outer join rentEquipment 
on Equipment.EquipmentID = rentEquipment.rEquipmentID left outer join Rental 
on Rental.RentalID = rentEquipment.rRentalID 
where typeName in (Select typeName from equipment where EquipmentID = '1')

Select * from Equipment left outer join rentEquipment 
on Equipment.EquipmentID = rentEquipment.rEquipmentID left outer join Rental 
on Rental.RentalID = rentEquipment.rRentalID 
where typeName in (Select typeName from equipment where EquipmentID = '1')
...
on Equipment.EquipmentID = rentEquipment.rEquipmentID left outer join Rental 
on Rental.RentalID = rentEquipment.rRentalID 
where typeName in (Select typeName from equipment where EquipmentID = '1') and hirefrom not in 
...
Select * from customer 
Delete from Customer where phone = 'a'

Select distinct branchName from Equipment
Select distinct hireFrom, count(hireFrom) from Rental
group by hireFrom

Select hireFrom from Rental

Select hireFrom from Rental
order by hireFrom

Select distinct TypeName from equipment where TypeName not in 
(select TypeName from equipment where branchName like 'Auckland%')
(Select TypeName from Equipment join rentEquipment on Equipment.EquipmentID = rentEquipment.rEquipmentID where rEquipmentID = '62')


Select * from Customer where fname like 'Benjamin'
